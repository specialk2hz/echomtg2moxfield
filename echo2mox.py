...
# Store the path to the auth token file
auth_token_file = 'auth_token'

# Fetch the token
auth_token = get_auth_token(auth_token_file)
...
if response.status_code == 200:
    ...
    # Define the functions for processing the card names and editions
    def remove_parentheses(text):
        return re.sub(r'\s*\(.*?\)\s*', '', text).strip()

    # Remove the JP Full Art on the NEO lands (optional)
    def remove_jp_full_art(name):
        return name.replace('- JP Full Art', '').strip()

    # Rename WC Gold Border Cards
    ...
    # Fix edition for specific cards for sucessful import
    ...
    for index, row in df.iterrows():
        ...
        # Step 2: Rename specific cards
        new_name = rename_cards(cleaned_name)

        # Step 3: Update the edition using the new name
        ...
        if original_name != new_name or original_edition != new_edition:
            ...
            print(f'Original: {original_name} (Edition: {original_edition}) -> New: {new_name} (Edition: {new_edition})')

        # Update the DataFrame with the modified values
        df.at[index, 'Name'] = new_name
        df.at[index, 'Edition'] = new_edition

    # Step 4: Save the modified DataFrame to a final CSV file
